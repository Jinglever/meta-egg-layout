...
func (s *GenderRepoImpl) GetIDBySemantic(smt string) uint64 {
	switch smt {
	case "MALE":
		return model.MetaGenderMale
	case "FEMALE":
		return model.MetaGenderFemale
	case "UNKNOWN":
		return model.MetaGenderUnknown
	default:
		return 0
	}
}

func (s *GenderRepoImpl) GetSemanticByID(id uint64) string {
	switch id {

...
type UserDetail struct {
	...
	UserPhones []*UserPhoneDetail `json:"user_phones"` // 用户电话列表
}
...
func (h *Handler) ToUserDetail(ctx context.Context, bo *biz.UserBO) (*UserDetail, error) {
	...
	var userPhones []*UserPhoneDetail
	for _, userPhoneBO := range bo.UserPhones {
		userPhoneDetail := &UserPhoneDetail{
			Id:          userPhoneBO.ID,
...
// 用户电话创建数据
type ReqCreateUserPhone struct {
	PhoneNumber string `json:"phone_number" binding:"required,max=20"` // 电话号码
}
...
type ReqCreateUser struct {
	...
	UserPhones []ReqCreateUserPhone `json:"user_phones"`                                      // 用户电话列表
}
...
type UserListInfo struct {
	...
	Name       *string              `json:"name"`        // 用户名 (nullable)
	Gender     uint64               `json:"gender"`      // 性别
	UserPhones []*UserPhoneListInfo `json:"user_phones"` // 用户电话列表
}
...
func (h *Handler) ToUserListInfo(ctx context.Context, objs []*biz.UserListBO) ([]*UserListInfo, error) {
	...
	for i := range objs {
		var userPhones []*UserPhoneListInfo
		for _, userPhoneBO := range objs[i].UserPhones {
			userPhoneListInfo := &UserPhoneListInfo{
				Id:          userPhoneBO.ID,
				UserId:      userPhoneBO.UserID,
				PhoneNumber: userPhoneBO.PhoneNumber,
			}
...
//	@Param		body			body		ReqCreateUserPhone	true	"用户电话"
//	@Success	200				{object}	RspData{data=UserPhoneDetail}
//	@Failure	400				{object}	RspBase
//	@Router		/api/v1/users/{user_id}/user_phones [post]
func (h *Handler) AddUserPhone(c *gin.Context) {
	userId := jgstr.UintVal(c.Param("user_id"))
	var req ReqCreateUserPhone
	err := shouldBind(c, &req)
	if err != nil {
		ResponseFail(c, err)
		return
	...
	err = h.BizService.AddUserPhone(ctx, userId, userPhoneBO)
	if err != nil {
...
//	@Param		Authorization	header		string	true	"Bearer <jwt-token>"
//	@Param		user_id			path		int		true	"用户ID"
//	@Param		userPhone_id	path		int		true	"用户电话ID"
//	@Success	200				{object}	RspBase
//	@Failure	400				{object}	RspBase
//	@Router		/api/v1/users/{user_id}/user_phones/{userPhone_id} [delete]
func (h *Handler) RemoveUserPhone(c *gin.Context) {
	userId := jgstr.UintVal(c.Param("user_id"))
	userPhoneId := jgstr.UintVal(c.Param("userPhone_id"))
	ctx := c.Request.Context()
	log := contexts.GetLogger(ctx).
		WithField("user_id", userId).
		WithField("userPhone_id", userPhoneId)
	err := h.BizService.RemoveUserPhone(ctx, userId, userPhoneId)
	if err != nil {
...
//	@Success	200				{object}	RspData{data=[]UserPhoneDetail}
//	@Failure	400				{object}	RspBase
//	@Router		/api/v1/users/{user_id}/user_phones [get]
func (h *Handler) GetAllUserPhone(c *gin.Context) {
	userId := jgstr.UintVal(c.Param("user_id"))
	ctx := c.Request.Context()
	log := contexts.GetLogger(ctx).WithField("user_id", userId)
	userPhoneBOs, err := h.BizService.GetAllUserPhone(ctx, userId)
	if err != nil {
		log.WithError(err).Error("BizService.GetAllUserPhone failed")
		ResponseFail(c, err)
		return
	}
	list := make([]*UserPhoneDetail, 0, len(userPhoneBOs))
	for _, userPhoneBO := range userPhoneBOs {
		detail := &UserPhoneDetail{
			Id:          userPhoneBO.ID,
			UserId:      userPhoneBO.UserID,
			PhoneNumber: userPhoneBO.PhoneNumber,
			CreatedBy:   userPhoneBO.CreatedBy,
